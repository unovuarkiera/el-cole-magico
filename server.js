...
//   BASE_URL               → https://el-cole-magico.onrender.com
//   EMAIL_FROM             → tu@email.com  (Gmail o SendGrid)
//   EMAIL_PASS             → contraseña de app Gmail
// ═════════════════════════════════════════════════════════════
...
// Precios en céntimos
const PRODUCTOS = {
  pdf:   { centimos: 499,  nombre: 'PDF Digital StoryOwl',   descripcion: 'Cuento personalizado en PDF — descarga instantánea' },
  libro: { centimos: 2499, nombre: 'Libro Físico StoryOwl',  descripcion: 'Cuento personalizado impreso en tapa dura — envío a domicilio' }
};

const BASE_URL = process.env.BASE_URL || 'https://el-cole-magico.onrender.com';


// ─────────────────────────────────────────────────────────────
// VALIDAR CUPÓN — lo valida directamente desde Stripe
// ─────────────────────────────────────────────────────────────
app.post('/validar-cupon', async (req, res) => {
  if (!stripe) return res.json({ ok: false, mensaje: 'Stripe no configurado aún' });

  const { codigo } = req.body;
  if (!codigo) return res.json({ ok: false, mensaje: 'Código vacío' });
  ...
  try {
    ...
    if (!promos.data.length) {
      return res.json({ ok: false, mensaje: 'Código no válido o expirado' });
    }
    ...
    if (coupon.percent_off) {
      descuento = coupon.percent_off;
      textoDesc = `${descuento}% de descuento aplicado`;
    } else if (coupon.amount_off) {
      // Convertimos importe fijo a porcentaje aproximado para el frontend
      // El descuento real lo calcula Stripe en el checkout
      textoDesc = `${(coupon.amount_off / 100).toFixed(2).replace('.', ',')} € de descuento aplicado`;
      descuento = -1; // señal de que es importe fijo — Stripe lo gestiona
    }

    if (coupon.percent_off === 100) textoDesc = '¡Cuento gratis! 🎉';

    return res.json({
      ok: true,
      descuento: coupon.percent_off || 0,
      mensaje: textoDesc,
      promoId: promo.id
    });

  } catch(e) {
    console.error('Error validando cupón:', e.message);
    return res.json({ ok: false, mensaje: 'Error al validar el código' });
  }
});
...
app.post('/crear-sesion-pago', async (req, res) => {
  if (!stripe) {
    return res.json({ error: 'Stripe no está configurado todavía. Añade STRIPE_SECRET_KEY en Render.' });
  }

  const { producto, codigo, tituloCuento, nombreNino } = req.body;
  const prod = PRODUCTOS[producto] || PRODUCTOS.pdf;
    ...
    const sessionParams = {
      ...
      line_items: [{
        price_data: {
          ...
          product_data: {
            name: prod.nombre,
            description: `${prod.descripcion}${tituloCuento ? ' — "' + tituloCuento + '"' : ''}`,
            images: [`${BASE_URL}/imagenes/og-storyowl.png`]
          },
          unit_amount: prod.centimos
        },
        quantity: 1
      }],
      // Campos de envío solo si es libro físico
      ...(producto === 'libro' ? {
      ...
      // Código promo si hay uno validado
      ...(codigo ? { discounts: [{ promotion_code: codigo }] } : { allow_promotion_codes: true }),
      ...
      success_url: `${BASE_URL}/pago-exitoso?session_id={CHECKOUT_SESSION_ID}&producto=${producto}`,
      cancel_url:  `${BASE_URL}/generador.html?pago=cancelado`,
      locale: 'es'
    };
...
// ─────────────────────────────────────────────────────────────
// PÁGINA DE ÉXITO — redirige tras pago completado
// ─────────────────────────────────────────────────────────────
...
<head>
  ...
  <title>¡Pago completado! — StoryOwl</title>
  <link href="https://fonts.googleapis.com/css2?family=Fraunces:ital,wght@0,700;1,700&family=Nunito:wght@400;700&display=swap" rel="stylesheet"/>
  <style>
    *{margin:0;padding:0;box-sizing:border-box;}
    body{background:#080318;color:white;font-family:'Nunito',sans-serif;
      min-height:100vh;display:flex;align-items:center;justify-content:center;text-align:center;padding:24px;}
    .card{background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);
      border-radius:28px;padding:52px 40px;max-width:500px;}
    .icon{font-size:64px;margin-bottom:20px;}
    h1{font-family:'Fraunces',serif;font-size:clamp(24px,4vw,36px);margin-bottom:14px;color:white;}
    p{color:rgba(255,255,255,.55);line-height:1.7;margin-bottom:8px;font-size:15px;}
    .highlight{color:#f7c94b;font-weight:700;}
    .btn{display:inline-block;margin-top:28px;background:linear-gradient(135deg,#f7c94b,#e5a800);
      color:#1a0840;font-family:'Fraunces',serif;font-weight:900;font-size:16px;
      padding:14px 36px;border-radius:50px;text-decoration:none;
      box-shadow:0 6px 24px rgba(247,201,75,.35);}
  </style>
</head>
<body>
  <div class="card">
    ...
    ${producto === 'pdf'
      ? `<p>Tu <span class="highlight">PDF personalizado</span> está en camino.</p>
         <p>Recibirás un email con el enlace de descarga en los próximos minutos.</p>`
      : `<p>Tu <span class="highlight">libro físico</span> está confirmado.</p>
         <p>Recibirás un email de confirmación con los detalles del envío.</p>
         <p>Tiempo estimado de entrega: <span class="highlight">5-10 días hábiles</span>.</p>`
    }
    <p style="margin-top:16px;font-size:13px;color:rgba(255,255,255,.3);">Si tienes alguna duda escríbenos a hola@storyowl.com</p>
    <a href="/generador.html" class="btn">✨ Crear otro cuento</a>
  </div>
</body>
</html>`);
});
...
// ─────────────────────────────────────────────────────────────
// WEBHOOK DE STRIPE — se ejecuta cuando el pago está confirmado
// Aquí enviamos el email con el PDF
// ─────────────────────────────────────────────────────────────
app.post('/stripe-webhook',
  ...
  async (req, res) => {
    ...
    if (evento.type === 'checkout.session.completed') {
      ...
      const producto = session.metadata?.producto || 'pdf';
      const email    = session.customer_details?.email;

      console.log(`✅ Pago completado: ${producto} para ${email}`);

      if (email && producto === 'pdf') {
        await enviarEmailPDF(email, session.metadata);
      } else if (email && producto === 'libro') {
        await enviarEmailConfirmacionLibro(email, session);
      }
    }
...
// ─────────────────────────────────────────────────────────────
// EMAIL — configuración nodemailer
// ─────────────────────────────────────────────────────────────
function crearTransporter() {
  return nodemailer.createTransport({
    ...
    auth: {
      ...
      pass: process.env.EMAIL_PASS   // contraseña de app Gmail (16 caracteres)
    }
  });
}

async function enviarEmailPDF(email, metadata) {
  try {
    const transporter = crearTransporter();
    const titulo = metadata.tituloCuento || 'Tu cuento personalizado';
    const nombre = metadata.nombreNino   || '';

    await transporter.sendMail({
      from:    `StoryOwl 🦉 <${process.env.EMAIL_FROM}>`,
      to:      email,
      subject: `🦉 Tu cuento "${titulo}" está listo para descargar`,
      html: `
        <div style="font-family:Georgia,serif;max-width:560px;margin:0 auto;background:#080318;color:white;border-radius:20px;overflow:hidden;">
          <div style="background:linear-gradient(135deg,#1a0840,#2a1060);padding:40px 32px;text-align:center;">
            <div style="font-size:52px;margin-bottom:12px;">🦉</div>
            <h1 style="font-size:28px;color:#f7c94b;margin:0;">¡Tu cuento está listo!</h1>
          </div>
          <div style="padding:32px;">
            <p style="font-size:16px;color:rgba(255,255,255,.8);line-height:1.7;">
              ${nombre ? `Hola,<br><br>El cuento personalizado para <strong style="color:#f7c94b;">${nombre}</strong> está listo.` : 'Hola,<br><br>Tu cuento personalizado está listo.'}
            </p>
            <p style="font-size:16px;color:rgba(255,255,255,.8);line-height:1.7;margin-top:16px;">
              <strong style="color:white;">"${titulo}"</strong>
            </p>
            <div style="margin:28px 0;text-align:center;">
              <p style="color:rgba(255,255,255,.5);font-size:14px;">
                Adjunto a este email encontrarás tu PDF en alta resolución, sin marca de agua, listo para imprimir o compartir.
              </p>
            </div>
            <p style="font-size:13px;color:rgba(255,255,255,.3);margin-top:24px;text-align:center;">
              ¿Tienes alguna pregunta? Escríbenos a hola@storyowl.com<br>
              <strong style="color:rgba(247,201,75,.5);">StoryOwl · storyowl.com</strong>
            </p>
          </div>
        </div>
      `,
      // NOTA: el PDF se adjunta aquí.
      // Por ahora el cliente genera el PDF en el navegador.
      // En la siguiente fase: generamos el PDF en el servidor y lo adjuntamos.
      // attachments: [{ filename: `${titulo}.pdf`, path: rutaPDF }]
    });
    console.log(`📧 Email PDF enviado a ${email}`);
  } catch(e) {
    console.error('Error enviando email PDF:', e.message);
  }
}

async function enviarEmailConfirmacionLibro(email, session) {
  try {
    const transporter = crearTransporter();
    const titulo = session.metadata?.tituloCuento || 'Tu cuento';
    const nombre = session.metadata?.nombreNino   || '';
    const envio  = session.shipping_details?.address;

    await transporter.sendMail({
      from:    `StoryOwl 🦉 <${process.env.EMAIL_FROM}>`,
      to:      email,
      subject: `📚 Pedido confirmado — Libro físico "${titulo}"`,
      html: `
        <div style="font-family:Georgia,serif;max-width:560px;margin:0 auto;background:#080318;color:white;border-radius:20px;overflow:hidden;">
          <div style="background:linear-gradient(135deg,#1a0840,#2a1060);padding:40px 32px;text-align:center;">
            <div style="font-size:52px;margin-bottom:12px;">📚</div>
            <h1 style="font-size:28px;color:#f7c94b;margin:0;">¡Pedido confirmado!</h1>
          </div>
          <div style="padding:32px;">
            <p style="font-size:16px;color:rgba(255,255,255,.8);line-height:1.7;">
              ${nombre ? `Tu libro personalizado para <strong style="color:#f7c94b;">${nombre}</strong>` : 'Tu libro personalizado'} 
              <strong style="color:white;">"${titulo}"</strong> está en producción.
            </p>
            ${envio ? `
            <div style="background:rgba(255,255,255,.05);border-radius:12px;padding:16px;margin:20px 0;">
              <p style="color:#f7c94b;font-weight:700;margin:0 0 8px;">Dirección de envío:</p>
              <p style="color:rgba(255,255,255,.7);margin:0;line-height:1.6;">
                ${envio.line1}${envio.line2 ? ', ' + envio.line2 : ''}<br>
                ${envio.city}, ${envio.postal_code}<br>
                ${envio.country}
              </p>
            </div>` : ''}
            <p style="font-size:14px;color:rgba(255,255,255,.5);">
              Tiempo estimado de entrega: <strong style="color:white;">5-10 días hábiles</strong>
            </p>
            <p style="font-size:13px;color:rgba(255,255,255,.3);margin-top:24px;text-align:center;">
              ¿Tienes alguna pregunta? Escríbenos a hola@storyowl.com<br>
              <strong style="color:rgba(247,201,75,.5);">StoryOwl · storyowl.com</strong>
    ...
    console.log(`📧 Email libro enviado a ${email}`);
  } catch(e) {
    console.error('Error enviando email libro:', e.message);
  }
}
